...
SDS_w_record = array(rep(0,sim*(p) ),c((p),sim))
SDS_beta_record  = array(rep(0,sim*(p+1)*length(Dset)),c((p+1),sim,length(Dset)))
...
T_SDS = matrix(rep(0,sim*length(Dset)),nrow =sim,ncol = length(Dset));
...
sigma_SDS = array( 0, c(length( test_v),length( test_v),sim,length(Dset)) ) ;
