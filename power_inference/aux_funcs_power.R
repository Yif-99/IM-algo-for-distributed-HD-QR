...
HBIC_conquer_ini <- function(lambda,pen,X,y){
  n = length(y)
  result = conquer.reg(X,y,lambda=lambda,tau=tau,kernel='Gaussian',penalty=pen
                       ,epsilon=0.001,iteTight=3)
  checkloss = sum(check(y-X%*%result$coeff[-1],tau=tau))
  hbic = log(checkloss) + sum(result$coeff[-1]!=0)*log(log(n))/n*1*log(p)
  return(list(hbic = hbic, model=result))
}
#
ini_est <- function(lam_seq){
  clusterExport(cl7, list( 'c0'))
...
HBIC_distlasso <- function(c0=0.5,lambda,betaini,T,verbose=FALSE){
  h = c0 * (hat_s_ini^2*log(p)/N)^(0.25);
  b = c0 * (hat_s_ini*log(p)/n)^(0.25);
  result = dist_SQR_uniform_L1(X,y,test_v,tau,beta=betaini,m,lambda,eta,G
                               ,s=hat_s_ini,h=h,b=b,c0=c0,phi=1.0, itermax=500,iterT=T,verbose)
  checkloss = sum(check(y-X%*%result$coeff,tau=tau))
  hbic = log(checkloss) + sum(result$coeff!=0)*log(log(N))/N*1*log(p)
  return(list(hbic = hbic, model=result))
}
...
#================
HBIC_distscad <- function(c0=0.5,lambda,betaini,T,ifora=FALSE,verbose=FALSE){
  ...
  result = dist_SQR_uniform_scad(X,y,test_v,tau,beta=betaini,m,lambda,eta,G
                                 ,h=h,b=b ,ifora=ifora,ora_supp=1*(beta0 != 0)
...
dist_scad_est <- function(c0=0.5,lam_seq,T,beta_pre){
  clusterExport(cl7, list("beta_pre","hat_s_ini",'ifora'))
  ret_conquer_seq = NULL
  ret_conquer_seq <- do.call(cbind, parLapply(cl7, lam_seq, function(lam) {
    HBIC_distscad(c0,lambda=lam,betaini=beta_pre,T=T,ifora)
  }))
  min_idx_conquer = which.min(ret_conquer_seq[1,])
...
smooth_score = function(X_l,y_l,beta,test_v,w,h){
  res = y_l - cbind(1,X_l)%*%beta
  res_w = X_l[,test_v] - cbind(1,X_l[,-test_v])%*%w
  result = mean((tau-1+pnorm(res/h))*res_w)
  return(result)
}

func_2_solve = function(x){
  h = 1.5*sd(y_master-cbind(1,X_master)%*%SDS_beta_mean)*n^(-1/3)
  beta_solve = SDS_beta_mean
  beta_solve[1+test_v] = x
  f = smooth_score(X_master,y_master,beta_solve,test_v,SDS_w_mean,h) + smooth_score(X,y,SDS_beta_mean,test_v,SDS_w_mean,h) - smooth_score(X_master,y_master,SDS_beta_mean,test_v,SDS_w_mean,h) 
  return(f)
}
...
library(nleqslv);library(rootSolve)
sourceCpp('../functions/dist_HD_QR_inference.cpp'); sourceCpp('../functions/generate_data.cpp')
c0 = 0.5; G = 20; eta = rep(7.5,G) ; etag = rep(4,G) 
...
vars_to_export <- list("HBIC_distlasso","HBIC_distscad","HBIC_global", "HBIC_conquer_ini"
                       ,'check',"G", "eta","etag", "p", 'check','test_v')
clusterExport(cl7, vars_to_export); clusterEvalQ(cl7, Rcpp::sourceCpp('../functions/dist_HD_QR_inference.cpp'))
clusterExport(cl7, list("m", "N", 'n','c0'))
clusterEvalQ(cl7, library("glmnet"))
####################################################
