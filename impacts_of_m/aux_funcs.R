...
HBIC_conquer_ini <- function(lambda,pen,X,y){
  n = length(y)
  result = dist_SQR_uniform_scad(X,y,tau,beta=beta_pre,M=1,lambda,2*eta,G
                                 ,s=-1,h=-1,b=-1,c0=c0,ifora=FALSE,ora_supp=1
                                 ,phi=1.0/2,itermax=300,iterT=3,verbose=FALSE)
...
ini_est <- function(lam_seq){
  ...
  lambda_ini = result_dist$pen_lambda 
  beta_ini = result_dist$coeff 
  return(beta_ini)
}
#========
...
HBIC_distlasso <- function(c0=0.5,lambda,betaini,T,verbose=FALSE){
  ...
  result = dist_SQR_uniform_L1(X,y,tau,beta=betaini,m,lambda,1*eta,G
                               ,s=hat_s_ini,h=h,b=b,c0=c0,phi=1.0/1, itermax=300,iterT=T,verbose)
  checkloss = sum(check(y-X%*%result$coeff,tau=tau))
  hbic = log(checkloss) + sum(result$coeff!=0)*log(log(N))/N*1.5*log(p)
  return(list(hbic = hbic, model=result))
}
...
HBIC_distscad <- function(c0=0.5,lambda,betaini,T,verbose=FALSE){
  ...
  result = dist_SQR_uniform_scad(X,y,tau,beta=betaini,m,lambda,1*eta,G
                                 ,s=hat_s_ini,h=h,b=b,c0=c0,ifora=FALSE,ora_supp=1*(beta0 != 0)
                                 ,phi=1.0/1,itermax=300,iterT=T,verbose=verbose)
  checkloss = sum(check(y-X%*%result$coeff,tau=tau))
  hbic = log(checkloss) + sum(result$coeff!=0)*log(log(N))/N*1.5*log(p)
  return(list(hbic = hbic, model=result))
}
...
####################################################
c0 = 0.8; G = 20; eta = rep(7.5,G) ; etag = rep(4,G) 
...
clusterExport(cl7, list('tau','c0','n'))
####################################################
